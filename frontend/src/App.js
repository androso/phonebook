...
const App = () => {
	...
	const [validPersons, setValidPersons] = useState(persons);
  	const [alreadyRemoved, setAlreadyRemoved] = useState({"status":false, "message": ""}); 
	...
	const submitPerson = (event) => {
      ...
      if (window.confirm(`${newName} is already in the phonebook, replace the old number with a new one?`)) {
		...
		const sendPerson = async () => {
			// We set {data} to make sure we only render the new person on the phonebook if the person was succesfully sent to the database
			const { data } = await axios.post(`${serverURI}/persons`, newPerson);
			setPersons([...persons, data]);
			setValidPersons((prevValid) => [...prevValid, data]);
			setNewName("");
			setNewPhoneNumber("");
			setIsSuccess(true);
			setTimeout(() => {
  ...
  const updateNumber = async (newPerson) => {
    const replacingPerson = persons.find(person => newPerson.name === person.name);

    const setNumber = async () => {
      
      await axios.put(`${serverURI}/persons/${replacingPerson.id}`, newPerson);
	...
	return (
		<div>
			<h2>Phonebook</h2>
      		{alreadyRemoved.state && <Notification type="error" message={alreadyRemoved.message}/>}
			{isSuccess && <Notification type="success" message={"Person was added succesfully!"}/>}
